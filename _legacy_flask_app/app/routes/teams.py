...
def apply_to_team(team_id):
    ...
    existing_application = TeamApplication.query.filter_by(teamId=team_id, userId=current_user.id).first()
    if existing_application:
        abort(400, description="You have already applied to this team")
    application = TeamApplication(userId=current_user.id, teamId=team_id, status='pending')
...
def follow_team(team_id):
    team = Team.query.get_or_404(team_id)
    if team in current_user.followingTeams:
        current_user.followingTeams.remove(team)
        db.session.commit()
        return jsonify({"success": True, "isFollowing": False})
    else:
        current_user.followingTeams.append(team)
        db.session.commit()
