...
3.  **Внимательно прочитайте последнюю строку ошибки.** Она точно указывает на причину.
    ...
    **Наш случай:**
    ...
    ```
    **Анализ:** Ошибка типов. Где-то в настройках таймаута ожидается число, а передается строка. Это произошло из-за параметра `?timeout=15` в `DATABASE_URL`, который некорректно обрабатывался драйвером `psycopg2` при работе через SOCKS-прокси.
