...
### 3.3. Временная структура на период миграции

Для безопасного и контролируемого перехода, мы изолируем старое Flask-приложение в отдельную директорию `_legacy_flask_app/`. Новое FastAPI приложение будет развиваться в основной структуре. Общие модули, такие как модели, будут использоваться обоими приложениями.

```
/
├── _legacy_flask_app/          # <-- ВСЁ СТАРОЕ FLASK-ПРИЛОЖЕНИЕ
│   ├── main_flask.py           # Бывший main.py
│   ├── __init__.py             # Старая фабрика app
│   ├── routes/                 # Старые blueprints
│   └── ...                     # Другие специфичные для Flask файлы
│
├── app/
│   ├── core/                   # <-- Новое ядро FastAPI
│   ├── routers/                # <-- Новые роутеры FastAPI
│   ├── schemas/                # <-- Новые Pydantic схемы
│   │
│   ├── models/                 # <-- ОБЩИЕ модели SQLAlchemy
│   └── services/               # <-- ОБЩИЕ или новые сервисы
│
├── main.py                     # <-- НОВАЯ точка входа FastAPI
├── requirements.txt            # Общий файл зависимостей
└── ...                         # Прочие общие файлы
```

**Преимущества:**
*   **Четкое разделение**: Исключает путаницу между старым и новым кодом.
*   **Простая очистка**: По завершении миграции достаточно удалить директорию `_legacy_flask_app/`.
